...
class VirtualHomee extends EventEmitter {
  constructor(homeeId, username, password, log) {
    super();
    ...
    try {
      mac = [].concat(...Object.values(networkInterfaces()))
        .filter((details) => details.family === 'IPv4' && !details.internal)
        .pop().mac;
    } catch (e) {
      log.warn('can\'t get mac address, using fallback');
      mac = '12:34:56:78:9a:bc';
    }

    this.AccessToken = shajs('sha256').update(mac).digest('hex');
  }
  ...
  start() {
    this.server.on('error', (err) => {
      if (err.code === 'EADDRINUSE') {
        this.log.error('Port 7681 is already in use. Please make sure that only one instance of the virtual homee is running.');
      } else {
        this.log.error(err);
      }
    });
