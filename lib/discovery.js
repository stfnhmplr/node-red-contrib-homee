 ...
 * @param {string} alias
 */
function start(alias, node) {
  server = dgram.createSocket({ type: 'udp4', reuseAddr: true });
  server.bind(15263);

  server.on('message', (message, rinfo) => {
    node.debug(`Discovery server receive message : ${message}`);
    ...
    // eslint-disable-next-line
      ...
      node.debug(`message did not match alias: ${message}__vs__${alias}`);
    }
  });

  server.on('listening', () => {
    const address = server.address();
    node.debug(`UDP Server started and listening on ${address.address}:${address.port}`);
  });
}
